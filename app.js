...
// Buffers editables eliminados (núcleos) para persistencia espejo
// Si se elimina un buffer de núcleo, también debe salir del análisis (globalData.selected)
let deletedEditableNis = new Set();
...
function saveBuffersState() {
  const state = {
    editableBuffers: [],
    customBuffers: [],
    deletedEditableNis: Array.from(deletedEditableNis || []),
...
function loadBuffersState() {
  try {
    ...
    // Restaurar lista de buffers de núcleo eliminados (persistencia)
    try {
      deletedEditableNis = new Set((state.deletedEditableNis || []).map(v => Number(v)).filter(v => Number.isFinite(v)));
    } catch (_) {
      deletedEditableNis = new Set();
    }
...
function normalizeImportedState(raw) {
  ...
  // Buffers de núcleo eliminados (opcional)
  try {
    const del = state.deletedEditableNis || state.deletedEditable || state.deletedNis;
    if (Array.isArray(del)) {
      out.deletedEditableNis = del.map(v => Number(v)).filter(v => Number.isFinite(v));
    }
  } catch (_) {}
...
function applyImportedStateToMap(state) {
  ...
  // 1b) Aplicar eliminaciones de buffers editables (núcleos)
  try {
    const delSet = new Set((state.deletedEditableNis || []).map(v => Number(v)).filter(v => Number.isFinite(v)));
    if (delSet.size) {
      delSet.forEach(ni => {
        // Eliminación espejo: quita el buffer y también lo saca de la cobertura
        deleteEditableBuffer(ni, { silent: true });
      });
    }
    // Mantener referencia global para persistir en futuros guardados
    deletedEditableNis = delSet;
  } catch (_) {}
...
function enableDeleteMode() {
  ...
  // También para buffers editables (núcleos) - permitir eliminar en modo libre
  editableBuffers.forEach((data, ni) => {
    data.circle.off('click');
    data.circle.on('click', (e) => {
      L.DomEvent.stopPropagation(e);
      if (deleteMode) {
        const name = (data?.nucleo?.name || `Núcleo ${ni}`);
        if (confirm(`¿Eliminar buffer de "${name}"?`)) {
          deleteEditableBuffer(ni);
        }
      }
    });
  });
...
function disableDeleteMode() {
  ...
  editableBuffers.forEach((data, ni) => {
    data.circle.off('click');
    data.circle.on('click', (e) => { L.DomEvent.stopPropagation(e); showBufferPopup(editableBuffers.get(ni), false); });
  });
}
...
// Elimina un buffer editable (de núcleo) y lo saca del análisis espejo
function deleteEditableBuffer(ni, opts = {}) {
  const data = editableBuffers.get(ni);
  if (!data) return;
  ...
  try {
    if (data.handle) {
      layers.bufferHandles.removeLayer(data.handle);
      data.handle = null;
    }
  } catch(_){ }

  try { layers.buffers.removeLayer(data.circle); } catch(_){ }
  editableBuffers.delete(ni);

  // Persistir eliminación
  try { deletedEditableNis.add(Number(ni)); } catch(_){ }

  // Quitar del análisis (fuente única: selected)
  try {
    if (globalData && Array.isArray(globalData.selected)) {
      globalData.selected = globalData.selected.filter(x => x !== ni);
    }
  } catch(_){ }

  if (!opts.silent) {
    try { showNotification("✓ Buffer eliminado", "info"); } catch(_){}
  }

  try { markAsChanged(); } catch(_){ }
  try { closeMetricsPanel(); } catch(_){ }
  try { debounceAnalyzeOrphans(); } catch(_){ }
}
window.deleteEditableBuffer = deleteEditableBuffer;
  ...
  const startDrag = (startEvent) => {
    ...
    const onDocUp = () => {
      ...
      if (overTrash) {
        ...
        // Núcleos: también se pueden eliminar (modo libre)
        if (!isCustom && ni !== null && ni !== undefined) {
          deleteEditableBuffer(ni);
          return;
        }
      }
...
function drawBuffersEditable(nucleos, selected, nucleoStats) {
  ...
  const deletedSet = new Set((savedState?.deletedEditableNis || []).map(v => Number(v)).filter(v => Number.isFinite(v)));
  // Mantener referencia global para próximos guardados
  try { deletedEditableNis = new Set([...deletedSet]); } catch(_) {}
  ...
  // Si hay eliminados persistidos, también deben salir del análisis espejo
  try {
    if (globalData && Array.isArray(globalData.selected) && deletedSet.size) {
      globalData.selected = globalData.selected.filter(x => !deletedSet.has(Number(x)));
    }
  } catch(_) {}

  selected.forEach(ni => {
    if (deletedSet.has(Number(ni))) return;
    ...
    circle.on('dblclick', (e) => {
      try { L.DomEvent.stopPropagation(e); L.DomEvent.preventDefault(e); } catch(_){ }
      if (!(editMode || deleteMode)) return;
      const name = (n?.name || `Núcleo ${ni}`);
      if (confirm(`¿Eliminar buffer de "${name}"?`)) deleteEditableBuffer(ni);
    });
